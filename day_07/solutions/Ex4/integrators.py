...
def explicit_euler_step(x_0,t,f,h):
    return x_0 + h * f(x_0, t)

def implicit_euler_step(x_0,t,f,h):
    # implicit euler step as solution of nonlinear equation: update(x) = 0
    update = lambda x : x_0+h*f(x,t+h) - x
    # use fsolve to find the solution of update(x) = 0
    x_hat = fsolve(update, x_0)
    return x_hat

def crank_nicolson_step(x_0,t,f,h):
    # explicit euler step to t+h/2
    x_ee = x_0 + h/2*f(x_0,t)
    # implicit euler step over interval [t+h/2,t+h]
    update = lambda x : x_ee + h/2*f(x,t+h) - x
    # fsolve to find the final update
    x_hat = fsolve(update, x_0)
    return x_hat

def heun_step(x_0,t,f,h):
    # evaluate the initial slope
    k_1 = f(x_0, t)
    # predict intermediate point
    x_ee = x_0 + h*k_1
    # evaluate slope at intermediate point
    k_2 = f(x_ee, t+h)
    # apply quadrature approximation
    x_hat = x_0 + h/2*(k_1+k_2)
    return x_hat


# add integrators one at a time after implementing 
# 
...
####################################
####################################
def integrate(f, x_0, tspan, h, int):
    t, tf = tspan
    x = x_0
    trajectory = [x_0]
    ts = [t]
